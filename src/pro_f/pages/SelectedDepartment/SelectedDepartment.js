...
const SelectedDepartment = () => {
  ...
  useEffect(() => {
    const fetchBudgetData = async () => {
      try {
        ...
        const response = await axios.post(url, { faculty_id: selectedFacultyId });
        ...
        const sortedTransactionData = transactionData.sort(
          (a, b) => new Date(b.faculty_payment_date) - new Date(a.faculty_payment_date)
        );
        ...
        const extractedYears = Array.from(
          new Set(sortedTransactionData.map(item => new Date(item.faculty_payment_date).getFullYear()))
        );
  ...
  const handleSearch = () => {
    const filtered = budgetData.filter(item => item.department_name.includes(programName));
    ...
    <div className="faculty-budget-container">
      ...
      <div className="faculty-select-wrapper">
        ...
        <select
          ...
          className="faculty-budget-select"
        >
      ...
      <div className="faculty-period-select">
        ...
        />
        <button className="faculty-department-search-btn" onClick={handleSearch}>
          검색
        </button>
      </div>
              ...
              <tr key={index}>
                {selectedOption === 'Mydex 온도 포인트 장학금' ? (
                  ...
                  <>
                    ...
                    <td className={item.faculty_used_budget < 0 ? 'faculty-negative' : 'faculty-positive'}>
